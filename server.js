...
const server = http.createServer((req, res) => {
    ...
    // 4 - GET Request - Return By Affiliation: Power
    if(req.url === '/affiliation/power' && req.method === 'GET') {
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
        let body = '';
        fsPromise.readFile(`${__dirname}/data/traveler-index.json`)
            .then((data) => {
                ...
                body += JSON.stringify(powerCharacters);
                fs.writeFile(`${__dirname}/data/powerCharacters.json`, JSON.stringify(powerCharacters), (err) => {
                    if (err) console.log(err)
                })
                res.write(body)
                res.end();
